...
$(document).ready(function(){
	 ...
	 */
	$( '.sk__submenu-ul' ).wrapInner( '<span className="nav-subwrap"></span>' );
	 ...
	 * className="sk__fade-in-7 duration-12".
	 */
		...
		$( $targetSelector ).each(function () {
			...
			if ( words.length ) {
				...
				wordsArr.forEach( function (item) {
					element.find( '.sk__visible' ).parent().append( '<span className="sk__clipped-text sk__gradient-back-v1">' + item + '</span>' );
				});
	...
	$( '.sk__project-gallery' ).each(function(i) {
		...
		$( 'body' ).prepend( '<div id="sk__parallax-gallery-nav-' + ( i + 1 ) + '" className="sk__parallax-gallery-nav"><div className="sk__parallax-gallery-nav-icon"></div></div>' );	
		...
		while ( n < numberOfItems + 1 ) {
			...
			$( '#sk__parallax-gallery-nav-' + ( i + 1 ) ).append( '<a id="sk__gallery-' + ( i + 1 ) + '-nav-link-' + n + '" href="#sk__gallery-' + ( i + 1 ) + '-item-' + n + '" className="sk__gallery-nav-link" data-gallery-item-number="' + n + '"></a>' );
	...
	$( '.sk__gallery-basic-thumbnails' ).each(function() {
		...
		if ( numb >= 1 ) {
			...
			$( gallery ).find( '.carousel' ).append( '<div className="carousel-indicators sk__flex-center"></div>' );
	...
	function iconboxCloneIcons() {
		$( '.sk__iconbox .sk__iconbox-icon-link > span.sk__iconbox-icon' ).each(function() {
			if ( $( this ).length ) {
				let i = 1;
				while (i < 4) {
					$( this ).clone().html( '<span className="sk__iconbox-trail"></span>' ).insertAfter( this );
					i++;
				}
			}
		});
	 ...
	 */
	$( '.sk__edge-beauty' ).prepend( '<div className="sk__edge-beauty-top"></div><div className="sk__edge-beauty-bottom"></div><div className="sk__edge-beauty-bottom-shadow"></div>' );
	...
	$( '.fancy-gradient-text-box h1.sk__gradient-fancy-text, ' +
		...
		if ( classes.length ) {
			...
			$( '<span className="' + newClasses + '"> ' + insideHTML + '</span>' ).insertAfter( this );
		}
	});
